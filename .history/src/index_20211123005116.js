...
const n_map = { "icon_day": 1, "icon_three_days": 3, "icon_week": 5 }

const Ttypes = { "icon_lectures": "Lecture", "icon_seminars": "Seminar", "icon_practicals": "Practical", "icon_tutorials": "Tutorial" };
...
for (let item of sidenavs) {
  item.addEventListener('click', (e) => {
    ...
    if (Ttypes[item.id] !== undefined) {
      // console.log(item.lastChild.textContent);
      Ttype = Ttypes[item.id];
      filterListener(ndays, Ttype);
    };
...
(async function () {
    ...
    var render_lists = function (lists) {
      ...
      for (let el of lists) {
        ...
        for (let t of el["tags"]) {
          ...
          li += `<button onclick="window.localStorage.setItem('selected-programme', '${json_data}');" class="m-1 shadow-sm text-link btn btn-sm"> ${t} </button>`
        }
    ...
    input.addEventListener('keyup', ()=>{
      filterProgrammes;
      document.getElementById("seacrch-area-dropdown").classList.remove("d-noe")
    });
