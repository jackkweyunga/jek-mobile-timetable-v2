...
const SelectionListener = (pid, pyear) => {
  startLoading();
  drawTimeTable(pid, pyear, ndays).then(() => {
    stopLoading();
  });
};
  ...
  var render_lists = function (lists) {
    ...
    for (let el of lists) {
      ...
      for (let t of el["tags"]) {
        ...
        li += `<button onclick="window.localStorage.setItem('selected-programme', '${json_data}');document.getElementById('selected-programme').innerText='${el["name"]}';window.location.reload();" class="m-1 shadow-sm text-link btn btn-sm"> ${t} </button>`
      }
