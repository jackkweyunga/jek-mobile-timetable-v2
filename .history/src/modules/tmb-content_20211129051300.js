...
import initialHtml from "./html/tmb-initial-contant.html";
...
export const drawTimeTable = async (pid, pyear, ndays = 5) => {
    ...
    Data(pid, pyear).then(data => {
        ...
        if (data !== null | undefined) {
            ...
            document.getElementById('selected-programme').innerHTML = data.name+` ${pyear}
                <br>
                A link to this specific timetable: <br>
                <a href='${window.location.href}/?id=${pid}&tag=${pyear.replace(" ", "-")}' ></a>'
                `;
            ...
            for (let c of for_color_codes) {
                ...
                course_taken_el += `
                    <li class="selected-course mt-2" style=""display:flex;flex-direction:column;>
                        <small class="selected-course">${c}</small>
                        <small>${longform}</small>
                    </li>
                `;
                bgcolors.push(show)
                k_++;
            ...
            for (let i = 0; i < ndays; i++) {
                ...
                innerCategories.classList.add("inner-categories");

                var cat_top_content = document.createElement("div");
                ["nav", "nav-tabs",]
                cat_top_content.classList.add("cat-top-content");

                top_content.appendChild(cat_top_content);


                var categoryTypes = ["Lecture", "Practical", "Tutorial", "Seminar"]

                for (let category of categoryTypes) {

                    // titles showing the category types
                    const cattitle = document.createElement("div");
                    cattitle.classList.add("cat-item");
                    cattitle.innerHTML = `
                    <label class="cat-title-item">${category}</label>
                    `;
                    cat_top_content.appendChild(cattitle)

                    var sect_column = document.createElement("div");
                    sect_column.classList.add("sect-column");
                    ...
                    for (let k = -1; k < 12; k++) {
                        ...
                        for (let period of data.events) {
                            ...
                            if ((period.day.slice(0, 3) === days[i]) && (period.type == category)) {
                                ...
                                if (k + 8 === start || k + 8 === end) {
                                    ...
                                    const height = ((end - start) * 100).toString()
                                    const top = ((start - 7) * 100).toString()
                                    ...
                                    lb.innerHTML = `
                                        ...
                                        ">
                                        <div style="font-size: 0.7rem;height: fit-content; margin: 1px;">${period.group} ${period.type} <br> ${period.room} <br>${start} - ${end} </div>
                                        </div>
                                        `;
                                    lb.style = `
                                        position: absolute;
                    ...
                    innerCategories.appendChild(sect_column);

                }

                day.appendChild(innerCategories)
