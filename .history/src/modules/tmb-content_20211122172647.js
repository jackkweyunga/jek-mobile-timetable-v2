    ...
    Data(pid, pyear).then(data => {
        ...
        if (data !== null | undefined) {
            ...
            for (let i = 0; i < ndays; i++) {
                ...
                cat_top_content.classList.add("nav");
                cat_top_content.classList.add("nav-tabs");
                cat_top_content.classList.add("nav-fill");

                day.appendChild(cat_top_content);


                var categoryTypes = ["Lecture", "Practical", "Tutorial", "Seminar", "Examination"]

                for (let category of categoryTypes) {

                    // titles showing the category types
                    const cattitle = period_box();
                    cattitle.style.minHeight = "35px";
                    cattitle.style.width = '';
                    cattitle.setAttribute("id", `${category}`)
                    cattitle.style.boxShadow = 'inset 0 0 0 #000';
                    cattitle.innerHTML = `
                    <label class="cat-title">${category}</label>
                    `;
